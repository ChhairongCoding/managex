...
class _SplashPageState extends State<SplashPage>
  ...
  void _navigateTo(String route) {
    ...
    // Wait minimum 3s for splash animation, then go
    final elapsed = _controller.lastElapsedDuration?.inMilliseconds ?? 0;
    final remaining = (3000 - elapsed).clamp(0, 3000);

    Future.delayed(Duration(milliseconds: remaining), () {
      if (!mounted) return;
      Navigator.pushReplacementNamed(context, route); // ✅ pushReplacement, not push
    });
  }
      ...
      body: BlocListener<RegisterBloc, RegisterState>(
        listener: (context, state) {
          if (state is Authenticated) {
            _navigateTo('/appPage'); // ✅ single navigation call
          } else if (state is Unauthenticated) {
            _navigateTo('/login');  // ✅ single navigation call
          }
        },
        child: Container(
